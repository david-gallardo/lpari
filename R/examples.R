...
available_lpari_examples <- function() {
  data.frame(
    name = c("old_faithful", "iris", "holzinger_swineford"),
    title = c(
    ...
    n = c(272L, 150L, 301L),
    p = c(2L, 4L, 9L),
    label = c(NA_character_, "species", "school"),
    role = c(
      "primary example",
...
#'
#' @param name One of `"old_faithful"`, `"iris"`, or
#'   `"holzinger_swineford"`.
#' @return A list with `data`, `indicators`, `label`, `id`, and `title`.
#' @export
load_lpari_example <- function(name = c("old_faithful", "iris", "holzinger_swineford")) {
  name <- match.arg(name)
...
#' @export
lpari_example <- function(name = c("old_faithful", "iris", "holzinger_swineford"),
                          n_starts = 25,
                          k = 1:5,
                          seed = 20260525,
                          ...) {
